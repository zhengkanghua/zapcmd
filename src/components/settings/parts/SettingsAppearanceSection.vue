    ...
    <div class="appearance-cards grid gap-[16px] min-[620px]:grid-cols-2 min-[620px]:items-start">
      ...
      >
        <div class="appearance-card__header flex items-baseline justify-between gap-[12px]">
          <h3 class="appearance-card__title m-0 text-[12px] font-[650] text-[var(--ui-text)]">
        ...
        <div class="appearance-card__body">
          <div class="theme-selector flex flex-wrap gap-[10px]">
            <button
              ...
              type="button"
              :class="[
                'theme-card flex cursor-pointer flex-col items-center gap-[8px] rounded-[var(--ui-radius)] border border-[var(--ui-settings-card-border)] bg-[rgba(var(--ui-text-rgb),0.015)] p-[12px] transition-[border-color,background,transform] duration-150 hover:border-[rgba(var(--ui-text-rgb),0.16)] hover:bg-[rgba(var(--ui-text-rgb),0.03)] hover:-translate-y-[1px]',
                {
                  'theme-card--active border-[var(--ui-accent)] bg-[rgba(var(--ui-brand-rgb),0.08)]':
            ...
            >
              <div class="theme-card__swatches flex gap-[4px]">
                <span
                  class="theme-card__swatch h-[24px] w-[24px] rounded-[6px] border border-[var(--ui-border)]"
              ...
              <span class="theme-card__name text-[12px] text-[var(--ui-text)]">{{ themeMeta.name }}</span>
            </button>
          </div>
        </div>
      </div>

      <div class="appearance-card appearance-card--effects grid gap-[10px] rounded-[14px] border border-[var(--ui-settings-card-border)] bg-[var(--ui-settings-card-bg)] p-[14px]">
        <div class="appearance-card__header flex items-baseline justify-between gap-[12px]">
          <h3 class="appearance-card__title m-0 text-[12px] font-[650] text-[var(--ui-text)]">
        ...
        <div class="appearance-card__body appearance-effects grid gap-[14px]">
          <div class="appearance-row grid grid-cols-[minmax(0,1fr)_auto] items-center gap-[12px] max-[520px]:grid-cols-1 max-[520px]:items-start">
            <div class="appearance-row__text grid gap-[4px]">
              <p class="appearance-row__label m-0 text-[12px] leading-[1.35] text-[var(--ui-subtle)]">
                {{ t("settings.appearance.blurLabel") }}
              </p>
              <p class="appearance-row__hint m-0 text-[11px] leading-[1.45] text-[var(--ui-settings-hint)]">
                {{ t("settings.appearance.blurHint") }}
              </p>
            </div>
            <div class="appearance-row__control flex items-center justify-end gap-[10px] max-[520px]:justify-start">
              <SToggle
                :model-value="props.blurEnabled"
                @update:model-value="emit('update-blur-enabled', $event)"
              />
              <span class="appearance-row__value text-[12px] text-[var(--ui-subtle)]">
      ...
      <div class="appearance-card appearance-card--preview grid gap-[10px] rounded-[14px] border border-[var(--ui-settings-card-border)] bg-[var(--ui-settings-card-bg)] p-[14px]">
        <div class="appearance-card__header flex items-baseline justify-between gap-[12px]">
          <h3 class="appearance-card__title m-0 text-[12px] font-[650] text-[var(--ui-text)]">
            {{ t("settings.appearance.preview") }}
          </h3>
          <span class="appearance-card__meta text-[12px] font-bold text-[var(--ui-subtle)] [font-variant-numeric:tabular-nums]">
            {{ percentDisplay }}
          </span>
        </div>
        <div class="appearance-card__body">
          <div
            class="appearance-preview-wrap grid h-[96px] w-full place-items-center overflow-hidden rounded-[10px] border border-[var(--ui-settings-row-border)] [background-image:var(--ui-settings-preview-checker)] [background-size:16px_16px]"
          >
            <div
              class="appearance-preview-panel flex h-[64px] w-[70%] flex-col items-center justify-center gap-[4px] rounded-[10px] border border-[rgba(var(--ui-text-rgb),0.14)] shadow-[0_4px_12px_rgba(var(--ui-black-rgb),0.25)]"
              :style="{ backgroundColor: `rgba(var(--ui-bg-rgb), ${props.windowOpacity})` }"
            >
              <span class="appearance-preview-text text-[16px] font-bold text-[var(--ui-text)]">ZapCmd</span>
              <span class="appearance-preview-sub text-[11px] text-[var(--ui-subtle)] [font-variant-numeric:tabular-nums]">
                {{ percentDisplay }}
              </span>
            </div>
          </div>
