...
- 桌面产品 UI 默认优先结合 `ui-ux-pro-max` 与 impeccable 项目技能集使用；其中 `ui-ux-pro-max` 负责设计系统、配色排版、交互与可达性约束，impeccable 负责视觉方向、反模板化与生产级精修。
- 做产品界面的首次设计、结构性改版或风格定调时，优先使用 impeccable 的 `frontend-design`；若项目根目录不存在 `.impeccable.md` 或缺少 `Design Context`，必须先运行 `teach-impeccable`，禁止凭代码实现反推用户、品牌和语气。
- 做现有产品界面的修改、优化、评审与质量提升时，优先使用 impeccable 的 `polish`、`critique`、`optimize` 等技能；如需补充设计系统、UX 规范、a11y 或栈建议，再结合 `ui-ux-pro-max`。
