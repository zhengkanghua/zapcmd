...
- 当前规划：启动 `v2.0`（主窗口 B4 UI 重构），设计基线见 `docs/ui-redesign/`
...
### Current Milestone: v2.0 主窗口 B4 UI 重构（当前里程碑）
...
- `drawerFloorHeight = 322px`：从矮窗口进入 Review 前补足左侧抽屉最小可视高度；仅允许 filler/spacer，禁止伪造假结果数据或 DOM。
...
- 下一步：`$gsd-plan-phase 13`（或先 `$gsd-discuss-phase 13`），从 Phase 13 开始进入执行
...
| B4 作为主窗口唯一结构方案（Overlay Review + Floor Height Protection） | 解决双焦点竞争、长命令窄栏可读性差与高度不稳定 | — Pending |
| `settings` 保持独立窗口，不并入 launcher | 避免范围膨胀；优先把主窗口壳层与交互契约做正确 | ✓ Good |
| `drawerFloorHeight = 322px` 仅通过 filler/spacer 实现 | 不污染键盘导航、可达性与测试语义 | — Pending |
| Review 的 dim/遮罩只作用于内部圆角 shell | 避免整窗遮罩暴露原生窗口边界与破坏圆角/透明外观 | — Pending |
| 动画策略：先试动态 resize，不稳则退回一次性 resize + 内部动画 | 在体验与稳定性之间保留可控回退策略 | — Pending |

---
*Last updated: 2026-03-07 after starting v2.0 milestone*
