...
统一口径：维护者个人私有文档只放一个目录，仓库不引用、不要求贡献者阅读。

1. 目录：`docs/.maintainer/`（本地私有，不入库）
2. 目录内分层：
   - `docs/.maintainer/requirements/`：需求/产品/架构类文档（长期有效）
   - `docs/.maintainer/work/`：发布跑书、排障记录、测试策略、临时工作文档（维护工作流）

（可选）从旧命名迁移（Windows PowerShell）：
说明：旧机器上可能存在 `docs/.local/` 或 `docs/.private/`，它们本质都是“维护者私有文档”。

```powershell
New-Item -ItemType Directory -Force docs/.maintainer/requirements | Out-Null
