...
## 补充（2026-03-18｜Settings UI 精修执行中）

- 已在 `feat/settings-ui-refinement-stable` worktree 起步：Task 1 先补稳定版窗口壳体 contract tests 并提交；Task 2 已切回 `decorations(true)`、移除自绘窗控 DOM、capability 收口到 close+updater，定向测试通过。
- 已完成 Task 4：补上 `--theme-settings-* -> --ui-settings-*` 映射，settings shell 改为 bounded responsive（普通页 720 / Commands 1120），并统一 `SSegmentNav`、Appearance、About 的稳定版结构与状态块样式。
- 已完成 Task 6：Hotkeys 改为行级 label + recorder 容器，冲突提示收口到字段局部；Commands toolbar 拆成 search-row / summary-row，宽表格与窄窗退化规则已在 `settings.css` 落地，相关 layout + 集成回归通过。
