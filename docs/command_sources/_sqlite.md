...
| 8 | `sqlite-table-info` | SQLite 表字段信息 | sqlite | all | `sqlite3 "{{file}}" "PRAGMA table_info('{{table}}');"` | file(path), table(text) | - | false | sqlite3 | sqlite pragma table_info schema observability 表结构 查看 show |
| 9 | `sqlite-index-list` | SQLite 索引列表 | sqlite | all | `sqlite3 "{{file}}" "PRAGMA index_list('{{table}}');"` | file(path), table(text) | - | false | sqlite3 | sqlite pragma index_list observability 索引 列表 查看 show |
| 10 | `sqlite-pragma-journal-mode` | SQLite Journal Mode | sqlite | all | `sqlite3 "{{file}}" "PRAGMA journal_mode;"` | file(path) | - | false | sqlite3 | sqlite pragma journal_mode observability journal 模式 查看 show |
| 11 | `sqlite-foreign-key-check` | SQLite 外键检查 | sqlite | all | `sqlite3 "{{file}}" "PRAGMA foreign_key_check;"` | file(path) | - | false | sqlite3 | sqlite pragma foreign_key_check observability 外键 检查 查看 show |
