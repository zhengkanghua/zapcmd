...
**Architecture:** navStack 在 `createLauncherRuntime()` 中创建，通过 `viewModel` 暴露，经 App.vue props drilling 传递到 LauncherWindow 后 provide 给子组件；新建 `LauncherCommandPanel.vue` 替代 `LauncherFlowDrawer.vue`；合并参数填写和高危确认为三场景合一面板；24h 免提示数据存 localStorage；队列批量执行的 safetyDialog 保留 `LauncherSafetyOverlay` 独立渲染。
...
| `src/composables/__tests__/launcher/useLauncherNavStack.test.ts` | 导航栈单元测试 |
...
| 文件路径 | 改动概述 |
|---------|---------|
| `src/composables/app/useAppCompositionRoot/runtime.ts` | navStack 在 `createLauncherRuntime()` 中创建并返回 |
| `src/composables/app/useAppCompositionRoot/viewModel.ts` | 暴露 navStack 相关字段（currentPage/canGoBack/pushPage/popPage/resetToSearch）到 viewModel |
| `src/App.vue` | 传递 navStack 相关 props 到 LauncherWindow |
| `src/components/launcher/LauncherWindow.vue` | 从 props 接收 navStack 并 provide；条件渲染 SearchPanel/CommandPanel；移除 FlowDrawer slot |
...
| `src/i18n/messages.ts` | 新增 12 个 commandPanel.* i18n key（顶级命名空间，与 launcher 平级） |
| `src/styles/launcher.css` | 新增 .command-panel* + .nav-slide-* 样式 |
| `src/features/hotkeys/__tests__/windowKeydownHandlers.test.ts` | paramDialogOpen+safetyDialogOpen → commandPanelOpen |
| `src/composables/__tests__/launcher/useMainWindowShell.test.ts` | pendingCommand/safetyDialog → navStack 引用 |
| `src/composables/__tests__/app/useAppWindowKeydown.test.ts` | paramDialogOpen/safetyDialogOpen → commandPanelOpen |

### 删除文件

| 文件路径 | 说明 |
|---------|------|
| `src/components/launcher/parts/LauncherFlowDrawer.vue` | 被 CommandPanel 替代 |
| `src/components/launcher/parts/LauncherParamOverlay.vue` | 旧参数弹窗，不再使用 |
| `src/components/launcher/parts/flowDrawerMotion.ts` | FlowDrawer 专属动画常量 |
| `src/components/launcher/parts/__tests__/LauncherFlowDrawer.test.ts` | 旧测试 |

> **注意：保留 `LauncherSafetyOverlay.vue`**。移除 FlowDrawer 后，队列批量执行（`executeStaged`）的 `safetyDialog` 仍需独立组件渲染。`LauncherSafetyOverlay` 仅用于此场景，在 `LauncherWindow.vue` 中根据 `safetyDialog` 状态条件渲染。
...
**Files:**
- Create: `src/composables/__tests__/launcher/useLauncherNavStack.test.ts`
...
运行: `npx vitest run src/composables/__tests__/launcher/useLauncherNavStack.test.ts`
预期: FAIL — `Cannot find module '../../launcher/useLauncherNavStack'`

- [ ] **Step 3: 提交测试文件**

```bash
git add src/composables/__tests__/launcher/useLauncherNavStack.test.ts
...
- [ ] **Step 1: 新增 `commandPanel` 顶级命名空间（与 `launcher`/`settings`/`safety` 平级），在 zh-CN 对象中添加以下嵌套对象**

```typescript
// 在 zh-CN 对象中新增顶级 commandPanel 命名空间（与 launcher 平级）：
commandPanel: {
  badge: {
    paramInput: "参数输入",
  ...
  danger: {
    title: "危险操作提醒",
    description: "此命令将直接操作敏感系统资源，请确保你知道该命令的作用范围后再继续执行。",
...
- [ ] **Step 2: 在 en-US 对象中添加对应 `commandPanel` 顶级命名空间嵌套对象**

```typescript
// 在 en-US 对象中新增顶级 commandPanel 命名空间（与 launcher 平级）：
commandPanel: {
  badge: {
  ...
  danger: {
    ...
    description: "This command directly operates on sensitive system resources. Make sure you understand its scope before proceeding.",
...
export interface LauncherCommandPanelProps {
  ...
  isDangerous: boolean;
  pendingArgValues: Record<string, string>;   // 来自 useCommandExecution，非本地状态
}
...
function mountPanel(props: Record<string, unknown>) {
  ...
  return mount(LauncherCommandPanel, {
    props: {
      pendingArgValues: {},  // 必须传入，复用 useCommandExecution 的状态
      ...props,
    },
    global: {
...
describe("LauncherCommandPanel", () => {
  ...
  describe("事件触发", () => {
    it("确认按钮触发 submit emit 并携带 pendingArgValues 和 dismissChecked", async () => {
      ...
      expect(submitEvents![0][1]).toBe(true); // dismissDanger = true
    });
...
// 参数 —— 复用 useCommandExecution 的 pendingArgValues（通过 props 传入）
// 而非组件内本地状态，确保与状态管理层一致
const args = computed<CommandArg[]>(() => getCommandArgs(props.command));
const hasArgs = computed(() => args.value.length > 0);
// pendingArgValues 和 updatePendingArgValue 从 props 获取
// props.pendingArgValues: Record<string, string>
// props.onArgInput: (key: string, value: string) => void
...
function onArgInput(key: string, value: string): void {
  // 调用 props 回调更新 useCommandExecution 的 pendingArgValues
...
### Task 10: runtime.ts + viewModel.ts + App.vue — navStack 创建与穿透

项目架构为 `runtime.ts → viewModel.ts → App.vue → LauncherWindow`（纯 props drilling），navStack 必须遵循此链路，不可在 LauncherWindow 中直接创建。
...
- [ ] **Step 1: runtime.ts — 在 createLauncherRuntime() 中创建 navStack**

在 `createLauncherRuntime()` 函数内创建导航栈实例，并将其包含在返回值中：

```typescript
import { useLauncherNavStack } from "../../launcher/useLauncherNavStack";

function createLauncherRuntime(context: AppCompositionContext) {
  // ... 现有的 stagingQueue, commandExecution, layoutMetrics, visibility ...
...
- [ ] **Step 2: viewModel.ts — 暴露 navStack 相关字段到 viewModel**

在 `createAppCompositionViewModel()` 返回值中暴露导航栈字段（展平为独立 props，保持与现有架构一致）：
...
- [ ] **Step 3: App.vue — 传递 navStack 相关 props 到 LauncherWindow**

在 `<LauncherWindow>` 模板绑定中新增导航栈 props：
...
预期: 无新类型错误（LauncherWindow props 定义会报缺失，下一 Task 修复）

- [ ] **Step 5: 提交**

```bash
git add src/composables/app/useAppCompositionRoot/runtime.ts src/composables/app/useAppCompositionRoot/viewModel.ts src/App.vue
git commit -m "feat(runtime): navStack 在 createLauncherRuntime 中创建，通过 viewModel 穿透到 App.vue"
```

---

### Task 11: LauncherWindow — 从 props 接收 navStack 并 provide + 条件渲染

LauncherWindow 从 App.vue 的 props 接收 navStack 字段，在组件内组装为 `LauncherNavStack` 对象并 `provide`，供子组件 inject 使用。

**Files:**
- Modify: `src/components/launcher/LauncherWindow.vue`

- [ ] **Step 1: 添加 import 和 provide（从 props 组装）**

在 `<script setup>` 中从 props 组装 navStack 对象并 provide：
...
import {
  ...
  type LauncherNavStack,
} from "@/composables/launcher/useLauncherNavStack";
import { dismissDanger } from "@/features/security/dangerDismiss";

// props 中新增 navCurrentPage, navCanGoBack, navPushPage, navPopPage, navResetToSearch, navStack
// 组装为 LauncherNavStack 对象供子组件 inject
const navStackProvided: LauncherNavStack = {
  stack: toRef(props, 'navStack'),
  currentPage: toRef(props, 'navCurrentPage'),
  canGoBack: toRef(props, 'navCanGoBack'),
...
<div class="launcher-nav-container" style="overflow: hidden; flex: 1; position: relative;">
  ...
  <!-- 队列批量执行的安全确认（safetyDialog 保留用于此场景） -->
  <LauncherSafetyOverlay
    v-if="props.safetyDialog"
    :safety-dialog="props.safetyDialog"
    @confirm="$emit('confirm-safety-execution')"
    @cancel="$emit('cancel-safety-execution')"
  />
</div>
```
...
git commit -m "feat(launcher-window): 从 props 接收 navStack 并 provide + CommandPanel 条件渲染"
```

---

### Task 12: SearchPanel — 移除 FlowDrawer 插槽 + 调整 flowOpen
...
### Task 13: useMainWindowShell — Esc 层级更新
...
注意：根据设计文档 §3.5 的 Esc 层级，执行流面板（`stagingExpanded` → staging drawer）优先于导航栈。此处 `stagingExpanded` 对应的是执行流抽屉面板，不是 FlowPanel 的 open/close。

- [ ] **Step 3: 更新调用方（runtime.ts `bindAppRuntime`）传入的 options**

修改 `src/composables/app/useAppCompositionRoot/runtime.ts` 中 `bindAppRuntime` 函数（约第 205-218 行）：

```typescript
// 旧：传入 pendingCommand / cancelParamInput / safetyDialog / cancelSafetyExecution
// 新：传入 navStack 相关参数
const shell = useMainWindowShell({
  // ... 保留现有的 query, stagingExpanded, closeStagingDrawer ...
  navStackCanGoBack: launcherRuntime.navStack.canGoBack,
  navStackPopPage: launcherRuntime.navStack.popPage,
});
```

> navStack 已在 `createLauncherRuntime` 中创建（Task 10），此处直接引用。

- [ ] **Step 4: 运行相关测试**

运行: `npx vitest run src/composables/__tests__/launcher/ --reporter verbose`
预期: 部分测试可能因 `useMainWindowShell` 接口变更而失败（`useMainWindowShell.test.ts` 的适配推迟到 Chunk 4 Task 19）
...
**3a. model.ts — 新增 `onNeedPanel` 回调到 `UseCommandExecutionOptions`：**

修改 `src/composables/execution/useCommandExecution/model.ts`，在 `UseCommandExecutionOptions` 接口中新增：

```typescript
// model.ts — UseCommandExecutionOptions 接口中追加：
onNeedPanel?: (command: CommandTemplate, mode: ParamSubmitMode) => void;
```

**3b. actions.ts — 添加 `needsPanel` 函数并修改 `executeResult`/`stageResult`：**

在 actions.ts 顶部添加辅助函数：
...
同理修改 `stageResult`（约第 188-194 行），将 mode 改为 `"stage"`。

**3c. runtime.ts — 接线 `onNeedPanel` 回调：**

在 `createLauncherRuntime()` 创建 `commandExecution` 时传入回调：

```typescript
const commandExecution = createCommandExecution({
  // ... 现有 options ...
...
### Task 15: windowKeydownHandlers — 适配导航栈
...
修改第 75-76 行，同时检查 `pendingCommand` **和** `safetyDialog`：
```typescript
...
> **关键**: 必须同时检查两个状态。`pendingCommand` 覆盖单命令 CommandPanel 场景；`safetyDialog` 覆盖队列批量执行安全确认场景。遗漏任一会导致热键在面板/对话框打开时仍然触发。
...
### Task 16: 删除旧文件（保留 SafetyOverlay 用于队列安全确认）
...
- **保留**: `src/components/launcher/parts/LauncherSafetyOverlay.vue`（队列批量执行的 `safetyDialog` 仍需此组件渲染）
...
> **注意**: 不删除 `LauncherSafetyOverlay.vue`。移除 FlowDrawer 后，队列批量执行（`executeStaged → checkQueueCommandSafety → safetyDialog`）仍需独立组件渲染安全确认。该组件在 Task 11 中已被集成到 `LauncherWindow.vue` 的导航容器内。

- [ ] **Step 2: 清理 import 引用**

搜索所有引用被删文件的代码，逐一修复：
```bash
grep -r "LauncherFlowDrawer\|LauncherParamOverlay\|flowDrawerMotion" src/ --include="*.ts" --include="*.vue" -l
```

- [ ] **Step 3: 提交**

```bash
git commit -m "chore: 删除旧 FlowDrawer/ParamOverlay 组件（保留 SafetyOverlay 用于队列安全确认）"
```

---

### Task 17: types.ts — 清理旧接口
...
### Task 18: 旧测试迁移 — LauncherWindow.flow.test.ts
...
### Task 19: 遗漏测试文件修改 — 适配导航栈 + commandPanelOpen
...
- [ ] **Step 1: windowKeydownHandlers.test.ts — 适配 commandPanelOpen**

更新测试中对 `paramDialogOpen` 和 `safetyDialogOpen` 的引用。将所有 mock 中的两个独立 boolean 替换为单一 `commandPanelOpen`：

```typescript
// 旧：
// paramDialogOpen: ref(false),
// safetyDialogOpen: ref(false),

// 新：
commandPanelOpen: ref(false),
```

同步更新测试断言：当 `commandPanelOpen = true` 时，热键应被抑制（与原来任一 `paramDialogOpen/safetyDialogOpen = true` 行为一致）。

- [ ] **Step 2: useMainWindowShell.test.ts — 适配导航栈**

更新测试中 `handleMainEscape` 的 mock options。将 `pendingCommand`/`cancelParamInput`/`safetyDialog`/`cancelSafetyExecution` 替换为：

```typescript
// 旧：
// pendingCommand: ref(null),
// cancelParamInput: vi.fn(),
// safetyDialog: ref(null),
// cancelSafetyExecution: vi.fn(),

// 新：
navStackCanGoBack: ref(false),
navStackPopPage: vi.fn(),
```

更新对应的测试用例：
- "有 pendingCommand 时 Esc 取消" → "导航栈 canGoBack 时 Esc popPage"
- "有 safetyDialog 时 Esc 取消" → 该场景由 `navStackCanGoBack` 统一覆盖
- 保留：stagingExpanded → closeStagingDrawer → 清空 query → hideMainWindow 的层级测试

- [ ] **Step 3: useAppWindowKeydown.test.ts — 适配 commandExecution mock 结构**

更新测试中 `commandExecution` mock 的 `pendingCommand` 和 `safetyDialog` 引用（注意：该测试文件传入的是原始 Ref，而非 computed boolean 名）。确保 mock 结构与 `useAppWindowKeydown.ts` 新接口一致——源码中 `commandPanelOpen` computed 已在 Chunk 3 Task 15 中完成，此处仅需测试侧 mock 匹配即可。

- [ ] **Step 4: 运行受影响的测试**

运行: `npx vitest run src/features/hotkeys/__tests__/ src/composables/__tests__/launcher/useMainWindowShell.test.ts src/composables/__tests__/app/useAppWindowKeydown.test.ts --reporter verbose`
预期: 全部 PASS

- [ ] **Step 5: 提交**

```bash
git add src/features/hotkeys/__tests__/windowKeydownHandlers.test.ts src/composables/__tests__/launcher/useMainWindowShell.test.ts src/composables/__tests__/app/useAppWindowKeydown.test.ts
git commit -m "test: 适配导航栈 + commandPanelOpen 的三个测试文件"
```
...
| 3. 集成 | Task 10-15 | Chunk 1 + 2 |
| 4. 清理 + 验证 | Task 16-20 | Chunk 1-3 |
